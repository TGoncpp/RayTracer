...
#pragma region TriangeMesh HitTest
		...
		inline bool SlabTestTriangleMesh(const TriangleMesh& mesh, const Ray& ray)
		{
		...
		{
			//slabtest
			if (!SlabTestTriangleMesh(mesh, ray))
			{
				return false;
			}

			HitRecord storredRecord{hitRecord};
			bool hit{ false };
			for (int i{}; i < mesh.indices.size() / 3; ++i)
			{
				...
				{
					hit = true;
					if (hitRecord.t > storredRecord.t)
						hitRecord = storredRecord;
				}
